...
import {
  obtenerVotanteDeStorage,
  eliminarVotanteDeStorage,
} from "../../utils/loginVotanteUtils";
    ...
    const fetchDatos = async () => {
      ...
      try {
        ...
        const resAsignacion = await fetch(
          `http://localhost:3000/votantes/asignacion/${credencial}`
        );
        ...
        const resVoto = await fetch(
          `http://localhost:3000/votantes/estado/${credencial}/${idEleccion}/${idCircuito}`
        );
        ...
        // 3. Estado de la mesa
        const resMesa = await fetch(
          `http://localhost:3000/eleccionCircuito/estadoMesa/${idEleccion}/${idCircuito}`
        );
    ...
    <div className="homevotante-container">
      ...
      <div className="main-content">
        ...
        {active === "bienvenida" && (
          <div>
            <p>
              <strong>Nombre:</strong> {votanteData?.nombre}{" "}
              {votanteData?.apellido}
            </p>
            <p>
              <strong>Credencial:</strong> {votanteData?.credencial}
            </p>
            <p>
              <strong>Estado del voto:</strong>{" "}
              {estadoVoto?.fueEmitido ? "Emitido" : "Pendiente"}
            </p>
            <p>
              <strong>Tipo de voto:</strong>{" "}
              {estadoVoto?.esObservado ? "Observado" : "Regular"}
            </p>
        ...
        {active === "estado" && estadoMesa && (
          <div>
            <p>
              ...
              {estadoMesa.mesaCerrada ? "Sí" : "No"}
            </p>
            <p>
              <strong>Habilitados para votar:</strong> {estadoMesa.habilitados}
            </p>
            <p>
              <strong>Votos emitidos:</strong> {estadoMesa.emitidos}
            </p>
          </div>
        )}
      </div>
